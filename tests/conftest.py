"""Shared pytest fixtures for the rrxiv-python test suite."""
...
def _no_real_client_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``rrxiv.client.client.sleep_for`` (a thin wrapper); mocking it
    keeps tests deterministic and fast even when the default retry
    policy backs off for several seconds.
    """
    monkeypatch.setattr("rrxiv.client.client.sleep_for", lambda _s: None)
